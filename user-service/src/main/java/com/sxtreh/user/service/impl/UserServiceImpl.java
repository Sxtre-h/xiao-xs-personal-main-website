package com.sxtreh.user.service.impl;
...
import com.sxtreh.user.mapper.UserMapper;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Long getUserRemainSpaceInfo(Long userId) {
        return userMapper.selectById(userId).getUserSpaceRemain();
    }

    @Override
    public void modifyUserNetDiskSpace(Long userId, Long decreaseSpace, String type) {
        if(type.equals(ModifyTypeConstant.INCREASE)){
            userMapper.increaseUserNetDiskSpace(userId, decreaseSpace);
        }
        else if(type.equals(ModifyTypeConstant.DECREASE)){
            userMapper.decreaseUserNetDiskSpace(userId, decreaseSpace);
        }
    }
    ...
    public void insertUser(UserDTO userDTO) {
        ...
        //为用户分配网盘空间
        user.setUserNetDiskRootId(100L);//临时分配一个空的目录ID
        ...
        Long rootCatalogId = netDiskClient.initialNetDiskRootCatalog(userFile);
        //将用户网盘根目录写入用户信息表
        System.out.println(rootCatalogId);
        user.setUserNetDiskRootId(rootCatalogId);
